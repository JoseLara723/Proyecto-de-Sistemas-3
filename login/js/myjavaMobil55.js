...
$(function(){
	...
	$('#rebusca').on('click', function(){
		...
		var url = 'login/php/ci_paginarCam.php';
		$.ajax({
		type:'POST',
		url:url,
		//data:'cdp='+cdp,
		data:'cdg='+cdg+'&cciu='+cciu+'&cpvv='+cpvv+'&cmo='+cmo,
		success: function(datos){
			$('#pagination').hide();
			$('#agrega-registros').html(datos);
		}
...
//var pregunta = confirm('¿Agregando..............?');
	var url = 'login/php/ci_agrega_Consulta1.php';
	$.ajax({
		type:'POST',
		url:url,
...
function mostrarfotopropi(id){ //sisadal
	...
	var url = 'login/php/ci_verpropi2.php';
	$('#myModal2').modal({
					show:true,
					backdrop:'static'
				});
...
function pagination(partida){ /// sidsadal
	var url = 'login/php/ci_paginarCambala.php';
	$.ajax({
		type:'POST',
		url:url,
