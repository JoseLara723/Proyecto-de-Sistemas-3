...
function agregaPago(){ // sisadal
	var url = '../php/cho_agrega_Caja.php';
	$.ajax({
		type:'POST',
		url:url,
...
//var pregunta = confirm('¿Esta seguro de eliminar este !!!!!!');
	$('#formulario3')[0].reset();
	var url = '../php/sho_editaAsigna2.php';
		$.ajax({
		type:'POST',
		url:url,
		data:'id='+id,
...
function VerPedido(id){ //SISADAL
	...
	var url = '../php/sho_VerPedidou2.php';
	//if(pregunta==true){
		$.ajax({
		type:'POST',
		url:url,
...
function pagination(partida){ /// sidsadal
	var url = '../php/sho_paginarCajau2.php';
	$.ajax({
		type:'POST',
		url:url,
