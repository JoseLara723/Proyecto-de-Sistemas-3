...
function agregaRegistro(){
	var url = '../php/sho_agrega_sucur.php';
	$.ajax({
		type:'POST',
		url:url,
...
function eliminarUsux(id){
	var url = '../php/sho_elimina_sucur.php';
	var pregunta = confirm('¿Esta seguro de eliminar Grupo ?');
	if(pregunta==true){
		$.ajax({
		type:'POST',
		url:url,
...
function editarUsuario(id){
	$('#formulario')[0].reset();
	var url = '../php/sho_edita_sucur.php';
		$.ajax({
		type:'POST',
		url:url,
		data:'id='+id,
...
function pagination(partida){
	var url = '../php/sho_paginarSucursal.php';
	$.ajax({
		type:'POST',
		url:url,
