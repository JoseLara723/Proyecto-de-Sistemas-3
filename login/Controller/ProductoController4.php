...
require_once '../Model/Producto4.php';
		...
		if (count($_SESSION['detalle'])>0) {
			...
			//	$idventa = $result_ultima_venta->ultimo;
				...
				foreach($_SESSION['detalle'] as $datos):
					...
					$objProducto->GuardarPastinas($idservicio, $descripcion, $cantidad, $precio, $subtotal,$nnota,$codigo,$foto,$codg,$grupo);
					$objProducto->ActualizarPastinas($idservicio, $cantidad);

				endforeach;
				$objProducto->Actualizarcodnu($nnota);

	$objProducto->guardarTotPastinas($observ,$ttotal,$nnota,$idusu,$grupo);
		...
		if (count($_SESSION['detalle'])>0) {
			...
			//	$idventa = $result_ultima_venta->ultimo;
				...
				foreach($_SESSION['detalle'] as $datos):
					...
					$ttotal=$ttotal+$subtotal;
					$objProducto->GuardarPastinas2($idservicio, $descripcion, $cantidad, $precio, $subtotal,$nnota,$codigo,$foto,$codg);
					$objProducto->ActualizarPastinas2($idservicio, $cantidad);

				endforeach;
				$objProducto->Actualizarcodnu($nnota);

				//$objProducto->guardarTotal($ttotal,$idventa,$encargado,$nnota);
	$objProducto->guardarTotPastinas2($observ,$ttotal,$nnota,$idusu);
