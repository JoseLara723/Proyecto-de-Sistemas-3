...
require_once '../Model/Producto5.php';
		...
		if (count($_SESSION['detalle'])>0) {
			...
			//	$idventa = $result_ultima_venta->ultimo;
				...
				foreach($_SESSION['detalle'] as $datos):
					...
					$objProducto->GuardarPegamento($idservicio, $descripcion, $cantidad, $precio, $subtotal,$nnota,$codigo,$foto,$codg,$grupo);
					$objProducto->ActualizarPegamento($idservicio, $cantidad);

				endforeach;
				$objProducto->Actualizarcodnu($nnota);

	$objProducto->guardarTotPegamento($observ,$ttotal,$nnota,$idusu,$grupo);
		...
		if (count($_SESSION['detalle'])>0) {
			...
			//	$idventa = $result_ultima_venta->ultimo;
				...
				foreach($_SESSION['detalle'] as $datos):
					...
					$ttotal=$ttotal+$subtotal;
					$objProducto->GuardarPegamento2($idservicio, $descripcion, $cantidad, $precio, $subtotal,$nnota,$codigo,$foto,$codg);
					$objProducto->ActualizarPegamento2($idservicio, $cantidad);

				endforeach;
				$objProducto->Actualizarcodnu($nnota);

				//$objProducto->guardarTotal($ttotal,$idventa,$encargado,$nnota);
	$objProducto->guardarTotPegamento2($observ,$ttotal,$nnota,$idusu);
