...
if (!isset($_SESSION)) {session_start();}
    ...
    $nroProductos = mysqli_num_rows(mysqli_query($conexion,"SELECT * FROM cambalache AS ca LEFT JOIN departamento AS de ON ca.coddep=de.coddep  WHERE ca.blkcam!='NO' "));
    $nroLotes = 3;
    ...
    if($paginaActual < $nroPaginas){
  	...
  	$registro = mysqli_query($conexion,"SELECT * FROM cambalache AS ca LEFT JOIN departamento AS de ON ca.coddep=de.coddep  WHERE ca.blkcam!='NO'  LIMIT $limit, $nroLotes ");
		...
		$tabla = $tabla.'<div class="row mb-4">
          <div class="col-md-12">
			...
			$tabla = $tabla.'<span class="property-location d-block mb-3"><span class="property-icon icon-pencil"></span> '.'Restantes :'. $op.' Dias'.'</span>';
				...
				if($moneda=='DO'){
                $tabla = $tabla.'<strong class="property-price text-primary mb-3 d-block text-success">$us :'.number_format($registro2['precioca'],0).'</strong>';
				}else{
                $tabla = $tabla.'<strong class="property-price text-primary mb-3 d-block text-success">Bs. :'.number_format($registro2['precioca'],0).'</strong>';
				}			
				
				
                $tabla = $tabla.'<ul class="property-specs-wrap mb-3 mb-lg-0">
                  <li>
